...
```DeferredTaskManager``` можно использовать как обычное хранилище событий, получая эвенты по требованию методом ```GetEventsAndClearStorage``` минуя раннеры, или осущесталять отправку доступных событий делегату по требованию с помощью метода ```SendEvents```.
